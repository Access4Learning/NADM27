...
namespace GenerateSpec.Generator
{
    ...
    public class GenerateXsd
    {
        ...
        public GenerateXsd(DocumentGlobalSettings documentGlobalSettings, XsdSettings XsdSettings, InputDocumentManager documentManager)
        {
            xsdSettings = XsdSettings;
            this.documentGlobalSettings = documentGlobalSettings;

            //Not sure what effect these lines have. They are not in the output. The first two are
            //also in InputDocumentManager. The third is now an input parameter. I think, but not positive
        ...
        private XsdSettings xsdSettings;

        /// <summary>
        /// 
        /// </summary>
        public XsdSettings GenerateXsdSettings
        {
            get { return xsdSettings; }
        }

        //VP
        //This property distinguishes whether the xsd definition is for a SIF object versus a type definition
        //or a code set, etc. 
        ...
        public void Generate()
        {
            ...
            //-------------------------------------
            //SIF_Message XSD Set (all elements set to optional)
            //-------------------------------------
            else 
            {
                GenerateRootXSD();
            ...
            if (xsdSettings.SingleSchema) EndXSDFile();
        }
        ...
        /// </summary>
        void GenerateRootXSD()
        {
            ArrayList includes = new ArrayList();

            //-------------------------------------------------------------------------
            //This generates the SIF_Message file.  Other items
            //such as common types are generated separately.
        ...
        void GenerateDataModelXSD()
        {
            ...
            while (groupIterator.MoveNext())
            {
                ...
                while (objectIterator.MoveNext())
                {
                    ...
                    if (!xsdSettings.SingleSchema) StartXSDFile(groupName, objectIterator.Current.GetAttribute("name", "").Trim(), null, namespaceIterator, importIterator);
        ...
        void GenerateCodeSetsXSD(XPathNodeIterator sourceIterator, string fileName)
        {
            ...
            while (sourceIterator.MoveNext())
            {
                ...
                if (!xsdSettings.SingleSchema) StartXSDFile(fileName, code);
        ...
        {
            //--------------------------------------------
            if (!xsdSettings.SingleSchema) StartXSDFile("", "CommonTypes");
            ...
            //-----------------------------------------------
            if (!xsdSettings.IsDataModelXSD && !xsdSettings.IsSIFMessage2XSD && mEmptyTypes.Count > 0)
            {
        ...
        void StartXSDFile(string folder, string name, string[] includes, XPathNodeIterator namespaceIterator, XPathNodeIterator importIterator)
        {
            ...
            if (!Directory.Exists(xsdSettings.OutputXSDPath + folder))
            {
                Directory.CreateDirectory(xsdSettings.OutputXSDPath + folder);
            }
            ...
            string path = xsdSettings.OutputXSDPath + folder + "\\" + tempName + ".xsd";
            ...
            if (!xsdSettings.AddNilAttributes)
            {
            ...
            */
            ...
            writer.WriteAttributeString("xmlns", documentGlobalSettings.SifNamespace);
            ...
            writer.WriteAttributeString("version", documentGlobalSettings.SchemaVersion);
        ...
        void GenerateDefinition(XPathNodeIterator rootIterator, bool isElement, bool isTyped)
        {
            ...
            */
            ...
            {
                if (rootNode != null && xsdSettings.Diagram)
                {
...
*/
                ...
                {
                    if (actionListConstraints || xsdSettings.ListWithKeysConstraints)
                    {
        ...
        void WriteCollectionDefinition(string name)
        {
            writer.WriteStartElement("xs:complexType");
                ...
                writer.WriteStartElement("xs:sequence");
                    writer.WriteStartElement("xs:element");
                        ...
                        writer.WriteAttributeString("type", name + "Type");
            ...
            writer.WriteStartElement("xs:element");
                ...
                writer.WriteAttributeString("type", name + "CollectionType");
            writer.WriteEndElement();
            WriteNewLine();
        }
        ...
        void WriteObjectKeys(string elementName, string[] keys)
        {
            ...
            if (xsdSettings.IsDataModelXSD || xsdSettings.IsSIFMessage2XSD || (inInfrastructure && !inDataObject))
            {
        ...
        void WriteListKeys(string elementName, string[] keys)
        {
            ...
            if (xsdSettings.IsDataModelXSD || (inInfrastructure && !inDataObject))
            {
                writer.WriteStartElement("xs:key");
            }
            else
        ...
        string GetListModePredicate()
        {
            ...
            if (xsdSettings.ListWithKeysConstraints)
            {
        ...
        {
            if (xsdSettings.AddNilAttributes && this.needNilAttribute)
            {
                ...
                if (lastSlash + 1 == level)
                {
                    ...
                    if (characteristicsIterator.MoveNext())
                    {
                        ...
                        { 
                            ...
                            if (this.inDataObject && !xsdSettings.IsDataModelXSD)
                            {
                                minOccurs = "0";
                            }

                            //make sure common elements in SIF_Message are optional
                            if (this.inCommonElement && !xsdSettings.IsDataModelXSD && !this.inInfrastructure)
                            {
                                minOccurs = "0";
                            }
                        ...
                        else if (characteristics == "O")
                        {
                            ...
                            {
                                if (!xsdSettings.AddNilAttributes) nillable = "true";
        ...
        void WriteType(object[][] itemArray, int index, string complexDerivation, bool suppress)
        {
            ...
            if (!isAttribute && xsdSettings.AddNilAttributes && this.needNilAttribute)
            {
                hasAttributes = true;
            }
            ...
            if (typeIterator.MoveNext())
            {
                ...
                if (definedIterator.MoveNext())
                {
                    ...
                    if (definedIterator.Current.LocalName == "any")
                    {
                        ...
                        if (hasAttributes && !xsdSettings.IsDataModelXSD && (inDataObject || (inCommonElement && !inInfrastructure)) && !xsdSettings.IsSIFMessage2XSD)
                        {
        ...
        void WriteSIFType(object[][] itemArray, int index, XPathNodeIterator item, XPathNodeIterator typeIterator, bool hasAttributes, bool suppress, string complex, string type, string typeAttribute)
        {
            ...
            if (!xsdSettings.IsDataModelXSD && !inInfrastructure)
            {
            ...
            if (hasAttributes)
            {
                ...
                if (xsdSettings.AddNilAttributes && this.needNilAttribute && IsComplexType(type))
                {
                    isComplex = true;
                }
                ...
                if (kludge && !xsdSettings.IsDataModelXSD && (inDataObject || (inCommonElement && !inInfrastructure)) && !xsdSettings.IsSIFMessage2XSD)
                {
                    type = type + "OrEmpty";
                }
            ...
            else
            {
                ...
                writer.WriteAttributeString(typeAttribute, type);
